...
md"""
##### Example: Direct Gibbs Energy Minimization of Multiple Chemical Reactions in Central Carbon Metabolism
...
begin
	...
	# G_formation_array[9] = -100000.0 	# 9 sink_ga3p
...
begin
	...
	if (should_run_this_block == true)
		...
		# setup bounds -
		LL = zeros(ℛ)
		#UL = maximum(n_initial_array)*ones(ℛ)

		# setup the objective function -
		OF_loop(p) = objective_function(p, parameters_dict)

		# setup defaul ic -
		xinitial_loop = zeros(ℛ)
		...
		for step_index = 1:number_of_steps
			...
			# call the optimizer -
    		opt_result_loop = optimize(OF_loop,LL, UL, xinitial_loop, Fminbox(BFGS()))
			ϵ_loop = Optim.minimizer(opt_result_loop)
			n_loop = n_initial_array + S*ϵ_loop

			for r_index = 1:ℛ
				eps_array[step_index,r_index] =  ϵ_loop[r_index]
			end

			for m_index = 2:ℳ
				n_final_array_loop[step_index,m_index] = n_loop[m_index-1]
			end
...
# ╔═╡ 0424ee3b-0517-4228-905c-2540e9ea3f95
