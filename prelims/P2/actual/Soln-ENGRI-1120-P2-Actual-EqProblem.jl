...
# ╔═╡ 930a9633-987d-46d2-94d0-bb9240608095
begin
	using PrettyTables
	using Plots
	using Optim
...
# ╔═╡ 51e77960-2e9b-4841-8d28-dc371fa9e8fb
md"""
#### Solution: Gas phase reaction equilibrium problem 
...
Initially, there are 100 mol of NH$_{3}(g)$ and no products in the reactor. 
...
# ╔═╡ 1d054b28-07a7-478e-b820-29261e99aa2d
begin
	# compute K -
	...
	# display the K_eq -
	with_terminal() do
		println("Check: K_eq = $(K_problem) from the KNOWN ΔG at T = $(T) K")
	end
end

# ╔═╡ bcfba1d8-7bd4-4a84-b948-ccf8d6aca55d
begin

	# residual array -
	residual_array = Array{Float64,1}()

	# ϵ array -
	ϵ_array = range(0.0,stop=0.99*n_initial[1],length=10000) |> collect
	for ϵ₁ in ϵ_array

		# compute final mol -
		n_final = n_initial + S*ϵ₁
		n_final = max.(0,n_final)
		...
		P_model = (1/V)*n_total*R_MPa*T
		P_term = (P_model/Pₒ)^2

		# compute K₁ -
		K₁ = (y_final[1]^-2)*(y_final[2])*(y_final[3]^3)
		K_model = P_term*K₁

		# compute error -
		error = (K_model - K_problem)^2

		# capture -
		push!(residual_array,error)
	end

	# find the min in the residual array -
	idx = argmin(residual_array)
	
	# compute the eq extent -
	ϵₑ = ϵ_array[idx]

	with_terminal() do
		println("Estimate of ϵₑ = $(ϵₑ) mol")
	end
end

# ╔═╡ a0ca7e91-3712-4892-b4f4-34305185248b
md"""
##### a) Compute the missing values in the state table
To compute the missing values in state table, we use the equilibrium extent $\epsilon_{1}^{\star}$ and the mol balances to compute the number of mol of component $i$ at equilibrium:

$$n_{i} = n_{i}^{\circ}+\sigma_{i1}\epsilon_{1}^{\star}\qquad{i=1,2,\dots,\mathcal{M}}$$

Then we _sum_ down the $n_{i}$ column to compute the total number of moles $n_{T}$. Finally, we know that $y_{i} = n_{i}/n_{T}$.


"""

# ╔═╡ e3394d2b-3976-4ed3-881f-61a08f88d1a3
begin
...
# ╔═╡ a29bcbd1-e0de-4665-aeb7-4d5ad3b21ffe
md"""
##### b) Compute the missing value for the equilibrium pressure $P$ (units: MPa)
"""

# ╔═╡ 9715d9b2-a87f-435e-bd2a-c9275d435b5c
begin
	...
	# display -
	with_terminal() do
		println("P_eq from IG = $(P_eq) MPa")
	end
end

# ╔═╡ afa84b1e-1254-45d3-b061-b6cef7467c86
md"""
##### c) Develop an analytical expression for the equilibrium constant
Starting from the general gas-phase equilibrium expression:

$$\begin{equation}
            \prod_{i=1}^{\mathcal{M}}\left(y_{i}\hat{\phi}^{v}_{i}\right)^{\sigma_{i1}} = \left(\frac{P}{P^{\circ}}\right)^{-\sigma_{1}}K
\end{equation}$$

First, we know from the assumptions that the gas phase is ideal, thus all the fugacity coefficients $\hat{\phi}^{v}_{i}\rightarrow{1}\forall{i}$. Next, run out the product and move the pressure term to the left-hand side:

$$\left(\frac{P}{P^{\circ}}\right)^{\sigma_{1}}\left[\left(y_{1}^{\sigma_{11}}\right)\times\left(y_{2}^{\sigma_{21}}\right)\times\left(y_{3}^{\sigma_{31}}\right)\right] = K$$

Finally, putting in the stoichiometric coefficients gives:

$$\left(\frac{P}{P^{\circ}}\right)^{2}\left[\left(y_{1}^{-2}\right)\times\left(y_{2}\right)\times\left(y_{3}^{3}\right)\right] = K$$

or

$$\left(\frac{P}{P^{\circ}}\right)^{2}\left[\frac{y_{2}y_{3}^{3}}{y_{1}^{2}}\right] = K$$

"""

# ╔═╡ 8a6623c2-2fdb-45c2-8385-8ae3f301ff0e
begin

	# compute the K from my expression -
	P_term = (P_eq/Pₒ)^2
	...
	# display -
	with_terminal() do
		println("Equilibrium constant estimated from the extent K₂ = $(K₂)")
	end
end

# ╔═╡ da217bbe-d8e9-4b5c-b031-7b67366e4542
md"""
...
# ╔═╡ f3f6b873-bb4f-4a8c-abe9-6fa2e4d0e32a
begin
	# compute the ΔG -
...
# ╔═╡ 46714b41-7cf3-405f-9ca6-ff8d9a1067c0
function compute_composition(ϵ, n_initial, S)
	...
	# compute K₁ -
	K₁ = (y_final[1]^S[1])*(y_final[2]^S[2])*(y_final[3]^S[3])
	K_model = P_term*K₁

	return (y_final, K_model)
end

# ╔═╡ beb9809c-4172-11ec-1871-7721c23bfd06
html"""
<style>
main {
    max-width: 1200px;
